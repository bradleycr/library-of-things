...
import { getCurrentPositionResult, type GeolocationResult } from "@/lib/geofence"
...
export default function ThingCheckoutPage({ params }: { params: Promise<{ uuid: string }> }) {
  ...
  const verifyLocation = async () => {
    setBusy(true)
    setLocation(await getCurrentPositionResult({ maximumAge: 0 }))
    setBusy(false)
  }
  ...
  const returnItem = async () => {
    ...
    try {
      const locationBody =
        location?.status === "success"
          ? {
              ...
              accuracy_m: location.coords.accuracyMeters,
      ...
      const body = await response.json()
      if (!response.ok) throw new Error(body.error ?? "Return failed")
      setComplete("return")
      await load()
    } catch (reason) {
  ...
  if (!payload) return <Message title="Opening item…" loading />
        ...
        <CardContent className="space-y-5">
          ...
          {isAvailable ? (
            ...
            <>
              ...
              {location?.status !== "success" && (
                <Button variant="outline" className="w-full" disabled={busy} onClick={verifyLocation}>
                  {busy && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              ...
              {location && location.status !== "success" && (
                <div className="space-y-3 rounded-lg border p-3">
                  <p className="text-sm text-muted-foreground">
                    Location could not be verified. You can confirm the physical return manually.
                  </p>
              ...
              <Button
                ...
                disabled={
                  busy ||
                  (!manualConfirm && location?.status !== "success")
                }
                onClick={returnItem}
              >
                Confirm return
