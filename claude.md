...
- **Home hero — Get library card** — `components/home-hero-actions.tsx` (client): shows **Get library card** → `/settings?mode=generate` next to Find a book / Add a book only after hydration when `useLibraryCard()` reports no stored card.
