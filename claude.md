...
- **Return flow** — My Books “Return” opens a dialog; user picks return node and optional **short note** (up to 200 characters — condition, how they enjoyed it, or anything else); note is stored with the return event and shown in sharing history. Checkout-page return (holder at node): same optional note field; geofencing uses ~1.5 km radius plus device-reported accuracy to reduce false “not nearby”; “Refresh my location” re-requests position.
...
- **Explore** — Bootstrap `error` state shown with retry; no location request (location used only for return flow). View toggles 44px touch targets; clear-search has `aria-label`. Supports `?node=<id>` to open filtered to a node (e.g. from homepage View Collection).
...
- **Node collections** — Homepage node cards include a **View Collection** button linking to `/explore?node=<id>`. Explore stays the same layout as before; the URL sets the Community Node filter so the list shows only that node’s books. No location request on explore; location is only used when returning a book (to allow return only when near a node).
...
- **Profile avatars** — DiceBear (dicebear.com) generates deterministic pixel-art avatars from a seed. Default seed is user id. Users can **regenerate** their profile image from Profile or Settings: “Regenerate profile image” → confirmation “Are you sure?” → `POST /api/users/[id]/regenerate-avatar` saves a new random `avatar_seed`; the new avatar is saved immediately (no separate “save” step). Regeneration can be done anytime. `users.avatar_seed` is added by `ensure-schema` if missing.
