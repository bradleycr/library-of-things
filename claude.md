...
- **ISBN-based checkout/return** — Optional feature (gated by `lib/feature-flags.ts`: `ISBN_CHECKOUT_RETURN_ENABLED`). Nav: “Scan to checkout or return”; book page: “Check out via ISBN scanner” / “Return via ISBN scanner” (holder); add-book success when “Do this later”: line about using scanner from menu. Scan → lookup by ISBN (bootstrap); **1 match** → redirect to that book’s checkout URL (navigation scheduled before closing dialog so the page opens); **2+** → copy picker then redirect; **0 matches** → “This book is not yet in the library. Would you like to add it?” with “Yes, add it” → `/add-book?isbn=...` (add-book prefills ISBN from URL and runs lookup). Checkout page when book is checked out and user is **not** the holder shows “This book is checked out” and “Contact the person who has it” (profile of holder) plus “View book details”. To disable: set flag to false.
