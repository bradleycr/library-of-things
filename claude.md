...
- **Library of Things:** trust-based sharing of books and useful physical objects at community nodes.
- Users get a **pseudonymous library card** (card number + PIN); borrower email is private and required by default unless an item opts out.
...
| `/steward/print-keycard-qr` | Steward sheet of 1″ or 1.5″ cut-out QR labels for every temporary keycard |
...
- **General items / guest loans:** `books.item_type` preserves existing URLs/FKs while supporting `keycard` and `other`. Checkout is email + one button. Return is one button after an NFC tap or QR scan (no email re-entry). Active guest email is shown on the tap page and as the steward holder name; erased on return. Same email may hold multiple keycards. Steward **Print QR labels** at `/steward/print-keycard-qr` (1″ or 1.5″ cut-outs).
...
- **CODE_OF_CONDUCT.md / SECURITY.md / docs/PRIVACY.md** — Collaboration expectations, vulnerability reporting, and public vs private data
...
- **docs/TEMPORARY_KEYCARDS.md** — Numbered temporary keycard provisioning, NFC, email privacy, and physical-presence return promise
...
- **Temporary keycard flow** — NFC or printed QR opens `/thing/[uuid]/checkout`. Enter email and check out; tap again to return. The card image and Available / Out status lead; a checked-out tap shows the holder email. Steward Temporary keycards panel can print 1″ / 1.5″ QR cut-outs. Missing/retired cards are labeled correctly (not as Available).
- **Temporary keycard returns** — No GPS and no email re-entry. Physical possession of the tag or QR is enough to return.
- **Book return geofencing** — Book returns may still request one location sample on user action (high-accuracy GPS and network/cell started together). Server recomputes distance with a configurable 3 km default + accuracy bonus (max 500 m). Books may move to a selected node. Missing/denied GPS or coarse accuracy permits explicit manual physical-return confirmation; out-of-range GPS with manual confirm still allows return. Raw coordinates are not retained.
...
- **ISBN scanner (`components/isbn-scanner-dialog.tsx`)** — Two-engine live scan (native `BarcodeDetector` on Android/Chrome; Quagga2 on iPhone Safari) plus photo decode. iPhone skips native probe and uses Quagga with stable back-camera selection (avoids macro/ultra-wide switching). Shared validation in `lib/isbn-utils.ts`; ISBN matching via `isbnMatches()` in `lib/isbn-checkout.ts`. Manual entry first; 2-read confirmation; Take photo stops live camera before decode.
...
- **Notify when available** — On book detail, “Notify Me When Available” is a coming-soon control (local preference / toast only); email delivery is not implemented yet. See `docs/NOTIFY_WHEN_AVAILABLE.md`.
...
- **Explore** — Bootstrap `error` state shown with retry. Explore and add-book do not request geolocation. Book **returns** may still request a one-shot location sample (see Book return geolocation). View toggles 44px touch targets; clear-search has `aria-label`. Supports `?node=<id>` to open filtered to a node (e.g. from homepage View Collection); when filtered, a banner shows the node name/address and **See all libraries** clears the filter. Availability filter label: **Available to borrow**.
...
- **Steward checked-out / overdue outreach** — Dashboard shows a **Currently checked out** table (holder, book, checkout date/time, how long out, email), sorted longest-out first; checkout time comes from the latest ledger `checkout`/`transfer` event. **Checked Out** and **Overdue** stat cards open detail dialogs with the same ordering plus **Copy all emails** (unique comma-separated list for To/BCC). Stats count catalog books only (temporary keycards stay in their own panel).
...
- **Node collections** — Homepage node cards include a **View Collection** button linking to `/explore?node=<id>`. Explore stays the same layout as before; the URL sets the Community Node filter so the list shows only that node’s books. Explore itself does not request location.
...
- **Book return geolocation** — `lib/geofence.ts` (haversine + parallel GPS/network lookup) and `lib/return-location.ts` (client preview copy). Book returns fetch location at submit on checkout/My Books flows. Temporary keycards use a physical-presence promise only (`/thing/[uuid]/checkout`).
