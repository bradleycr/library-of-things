...
- **ISBN scanner (`components/isbn-scanner-dialog.tsx`)** — Two-engine, single-UI scanner used everywhere ISBNs are read (add-book, my-books return gate, book page check-out/return, global Scan to checkout/return).
  - **Engine 1 (preferred): native `BarcodeDetector`** when the browser supports `ean_13`. iOS 17+ Safari, modern Chrome/Edge. Hardware-accelerated; dramatically more reliable than Quagga on iPhone. Component renders its own `<video>` element with `playsinline` + `muted` so iOS Safari doesn't fullscreen.
  - **Engine 2 (fallback): Quagga2** with conservative iOS-safe config — **no** `advanced: [{ focusMode: "continuous" }]` constraint (it was causing iPhones to thrash autofocus and switch between back cameras), modest `640×480` ideal resolution (biases iOS toward the main wide camera with best close-focus), default `patchSize: "medium"`, EAN-13 only, no `area` constraint (UI scan-guide rectangle is now purely informative).
  ...
  - **Safety nets** (both engines + photo): EAN-13 check digit, Bookland prefix `978`/`979`, double-read confirmation (2 same valid reads within ~4 s, ~600 ms warmup). Invalid frames between valid reads don't reset the counter — autofocus hunting doesn't break the double-read.
  ...
  - **UX**: manual ISBN field is rendered first (nudges the easiest path). When the Quagga fallback engine is active, the status line shows a hint to try Take photo if live doesn't pick up. Dialog is responsive (`max-w-[calc(100vw-2rem)]`, `min-w-0`).
