...
- **Temporary keycard flow** — A physical tag opens `/thing/[uuid]/checkout`; email alone signs out a temporary keycard without creating a member. The private email is never exposed in bootstrap/ledger and is erased at return. A second tap from the same browser opens return; a steward recovers lost sessions. Steward Temporary keycards panel syncs home-node Select after bootstrap loads; missing/retired cards are labeled correctly (not as Available).
- **Geofenced returns** — Book and temporary-keycard returns request one location sample on user action (high-accuracy GPS and network/cell started together so iPhone Safari still treats both as a user gesture). Server recomputes distance with a configurable 3 km default + accuracy bonus (max 500 m). Temporary keycards return to their immutable home node; books may move to a selected node. Missing/denied GPS or coarse accuracy permits explicit manual physical-return confirmation; out-of-range GPS with manual confirm still allows return. Raw coordinates are not retained.
...
- **Return geolocation** — `lib/geofence.ts` (haversine + parallel GPS/network lookup) and `lib/return-location.ts` (client preview copy). Keycard return (`/thing/[uuid]/checkout`) fetches a fresh location on **Confirm return** (optional **Check location** preview); manual confirmation always available with iPhone Settings guidance when permission denied. Book returns fetch location at submit on checkout/My Books flows.
