...
- **Steward checked-out / overdue outreach** — Clicking the **Checked Out** or **Overdue** stat card opens a dialog listing those books with holder name (profile link) and contact. Both dialogs show a **comma-separated unique email list** and a **Copy all emails** button so stewards can paste into To/BCC for a return reminder. Per-row copy for email/phone still available. Stats count catalog books only (temporary keycards stay in their own panel).
